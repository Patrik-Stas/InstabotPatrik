...
class ItShouldUpdateExistingUserInDBWhoseMediaWasLiked(CoreDBInteractionsTestCase):
    ...
    def runTest(self, mock_tools):
        ...
        self.assertEqual(11, owner_user.count_likes_we_gave)
        self.assertEqual(like_time, owner_user.dt_like)
...
class ItShouldUpdateDbWhenNewUserInformationAvailable(CoreDBInteractionsTestCase):
    def runTest(self):
        ...
        self.repo_bot.update_user(instabotpatrik.model.InstagramUser("userid1337", bot_data=bot_data))
        ...
        self.assertEqual(user.url, db_user.url)
        self.assertEqual(user.count_shared_media, db_user.count_shared_media)
        self.assertEqual(user.count_follows, db_user.count_follows)
        self.assertEqual(user.count_followed_by, db_user.count_followed_by)
        self.assertEqual(user.we_follow_user, db_user.we_follow_user)
        self.assertEqual(user.user_follows_us, db_user.user_follows_us)
        self.assertEqual(user.count_likes_we_gave, db_user.count_likes_we_gave)
        self.assertEqual(user.dt_like, db_user.dt_like)
        self.assertEqual(user.dt_follow, db_user.dt_follow)
        self.assertEqual(user.dt_unfollow, db_user.dt_unfollow)
...
class ItShouldCorrectlyPersistUserDataInDb(CoreDBInteractionsTestCase):
    ...
    def runTest(self):
        ...
        instabotpatrik.model.InstagramUser("userid1337", bot_data=bot_data, username="username1337")
        ...
        self.assertEqual(user.we_follow_user, db_user.we_follow_user)
        self.assertEqual(user.user_follows_us, db_user.user_follows_us)
        self.assertEqual(datetime.datetime(2012, 10, 1, 2, 0, 0, tzinfo=pytz.UTC), db_user.dt_like)
